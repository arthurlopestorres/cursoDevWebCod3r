...
const destaque = (seletor, status) => {
    status === 'true' ? Array.from(document.querySelectorAll(seletor)).forEach(element => element.classList.add('destaque')) : Array.from(document.querySelectorAll(seletor)).forEach(element => element.classList.remove('destaque'))
}
