...
//? 1- a primeira coisa que coisa que preciso fazer quando estou construindo um aplicativo que tem comunicação com rede, preciso declarar uma porta.
...
//? 2- vamos agora carregar o express, que é o arquivo que está node_modules
const express = require('express')
const app = express() //* -> preciso passar o express para dentro de uma variavel para ter acesso ao retorno, ou posso executa-lo na declaração para receber o objeto da função factory que ele abriga.
...
//? 3- para começar o serviço, vamos fazer uma requisição get. O get terá como parametro requisição, resposta, next(que é a função seguinte - callback):
//* app.get('/produtos', (req, res, next) => {
...
//? 1- primeiro, criamos o arquivo 2_bancoDeDados.js (para persistir os dados)
//? 2- Depois configuramos criando um objeto com função get para incrementar o id de produtos (gerando um id novo para cada produto)
//? 3 - Depois, configuramos uma função que é resposável por receber o produto sendo cadastrado, atribuir a ele um ID (caso ele não tenha) e injetar o produto no objeto 'produtos' para persisti-lo
//? 4- Depois, configuramos uma função getProduto(id) que tem como objetivo retornar o produto a partir do id
//? 5- Depois, congiguramos uma função getProdutos() que tem como objetivo retornar todos os produtos da listagem (todos os produtos cadastrados)
//? 6- Depois, configuramos o module.exports = {} que exporta as 3 funções mencinadas acima
...
//? 1 - No terminal, eu enviei o comando 'npm i --save body-parser@1.18.2 -E' para salvar esta dependencia (body-parser).
//? 2 - Importei aqui para o documento o body parser com o require(body-parser)
//? 3 - Depois passei para o express utilizando o metodo usei o bodyParser.urlencoded({extended: true}) que faz com que o recebimento de informações seja feita da forma correta (aplicando JSON.parse no recebimentos dados no server). O parametro "extended: true" significa que ele vai obrigatoriamente tratar o dado de todas as requisições feitas com body (do tipo post, por exemplo)
//? 4- Depois criei neste documento um metodo .put do express, para substituir dados de produtos salvos anteriormente.
//? 5- Depois, acessei o banco de dados e criei uma função de exclusão de produtos, que seria o método 'delete' do express.
